...
**상황**: 사용자가 비용 분석이나 원화(KRW) 환산 금액을 요청

**Skills 조합**:
1. `finance` 스킬 참조
2. `get_exchange_rate` 도구로 환율 조회
3. 리포트 생성 시 환율 정보 및 환산 금액 포함

**예시**:
```
사용자: "오늘 환율로 계산해서 리포트 만들어줘"
→ get_exchange_rate("KRW") 실행
→ reporting 스킬 참조하여 금액 변환
→ 리포트에 "환율 정보: 1 USD = 1,400 KRW (예시)" 포함
```
